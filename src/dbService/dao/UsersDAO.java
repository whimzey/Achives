...
public class UsersDAO {
    ...
    public UsersDataSet getFullDataSet(UsersDataSet dataset)  {
        
    	String boss = null;
		try {
			boss = executor.execQuery("select * from boss where idboss=" + dataset.getBoss(),result-> {
			    result.next();
        ...
        for (int i = 0; i < achives.length; i++) {
        	try {
				...
				System.out.println(222222);// TODO Auto-generated catch block
				e.printStackTrace();
			}
	}
    ...
    public BossDataSet getBossProfile(String name, String pass) throws SQLException {

        return executor.execQuery("select * from boss where usernameBoss='" + name+ "' and passwordBoss='"+pass+"'", result -> {
            result.next();
...
public ArrayList<String> getAchivesListbyId(long id) throws SQLException{
    ...
    public long getUserId(String name, String pass) throws SQLException {
        return executor.execQuery("select * from users where username='" + name + "' and password='"+pass+"'", result -> {
            result.next();
	...
	public void addRecievedAchives(long idUser, String achive) throws SQLException {
		...
		if(achives.equals("0")){achives = String.valueOf(achivevalue);}
		else{
		achives+=","+achivevalue;}
	...
	public void minusScore(String price, Long id) throws SQLException {
	
		int score = executor.execQuery("select * from users where idUsers='" + id + "'", result -> {
            result.next();
            return result.getInt(8);});
		
		score = score - Integer.parseInt(price);
		executor.execUpdate("update users SET score='"+score+"' WHERE idUsers='"+id+"'");
		
	}
	public ArrayList<String> getBosses() throws SQLException{
	...
	public boolean checkUser(String login) throws SQLException  {
		String	username = executor.execQuery("select * from users where username='" + login + "'", result -> {
			if(result.next()){
				return   result.getString(2);}  
		else { return "";}});
		
				String usernameBoss = executor.execQuery("select * from boss where usernameBoss='" + 
			login + "'", result -> {
				if(result.next()){
					return   result.getString(2);}  
			else { return "";}});
			
		if(username.equals("")&usernameBoss.equals("")){
		return true;	
		}
		else{return false;}
	...
	public void addNewAchive(String achive, String price, String bossid) throws SQLException {
		
		executor.execUpdate("insert into achives (Name, BossId, value) values "
				+ "('" + achive + "','" + bossid + "','" + price + "')");
